...
export default function PlanSelectionPage() {
  ...
  useEffect(() => {
    if (!isAuthenticated) {
      router.push('/login?redirect=/onboarding/plans');
      return;
    }
    if (!user) return;
    const step = user.onboarding_step;
    if (step === 'verify_email') router.push('/onboarding/verify');
    else if (step === 'setup_studio') router.push('/onboarding/setup');
    else if (step === 'complete' || !step) router.push('/dashboard');
  ...
  const handleSelectPlan = async (planName: string) => {
    ...
    try {
      ...
      }>('/auth/select-plan', { plan_id: planName });

      updateUser({ onboarding_step: res.onboarding_step });
      toast.success(`${planName.charAt(0).toUpperCase() + planName.slice(1)} plan selected!`);
  ...
  if (isLoading) {
    return (
      <OnboardingLayout currentStep={2} maxWidth="900px">
    ...
    <OnboardingLayout currentStep={2} maxWidth="900px">
      <div className="mb-7">
        ...
        <p className="mt-1 text-[13px] text-muted-foreground">
          Step 3 of 4 — Select a plan to get started. You can change it
          anytime.
        </p>
      </div>
            ...
            >
              ...
              <div className="space-y-1.5 mb-4 text-[11px]">
                ...
                <div className="flex items-center gap-2 text-muted-foreground">
                  ...
                  <span>
                    {plan.max_staff >= 999 ? 'Unlimited' : plan.max_staff}{' '}
                    staff
                  </span>
                </div>
                <div className="flex items-center gap-2 text-muted-foreground">
      ...
      <p className="text-center text-[11px] text-muted-foreground mt-6">
        All plans include a free trial period. No payment required to get
        started.
      </p>
    </OnboardingLayout>
  );
