...
## ACCURATE remaining Phase-6 inventory (15 services still on legacy PrismaService, 2026-06-14)
Measured via `grep -rln "import { PrismaService }" src --include=*.service.ts | grep -v spec`.
Per module (count): **auth 10** (incl. the deferred onboarding tenant-write — organization/branch/staff via runInTenantContext), member/data 2 (the deferred cross-tenant: member-discovery, member-context), referrals 1 (re-check member-referrals), prisma 1 (legacy PrismaService itself — stays until cutover), member/directory 1 (member-directory.backfill cross-tenant).
> The 4 non-auth, non-deferred ones (referrals re-check + the legacy PrismaService stay-put) aside, **the remaining real work is concentrated in `auth` (10) + the deferred cross-tenant trio**. auth is a CLAUDE.md HARD-STOP zone (identity/tenant-provisioning) and includes the onboarding tenant-write keystone — do it as its own carefully-reviewed slice, not a bulk swap.
...
> Domains migrated so far (~116 services, 43 commits): auth-registry, settings, members, classes, inventory, check-ins, member-BFF (minus cross-tenant trio), dashboards, referrals, payments, staff, organization, branches, marketing, analytics, events, documents, subscription, search, roles, onboarding, compliance, audit, ai, common/services, **platform/services**. **Still TODO:** auth (10, HARD STOP), member/data cross-tenant trio (2) + member-directory.backfill (1), referrals (1 — re-check), SCC; the auth onboarding tenant-write; the non-service legacy users (guards/middleware/queue-processors/controllers); then Phases 7–10.
