...
ALTER ROLE musclex_app NOSUPERUSER NOCREATEDB NOCREATEROLE NOBYPASSRLS;
...
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA studio_template TO musclex_app;
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA public         TO musclex_app;
...
DECLARE r RECORD;
BEGIN
  SELECT rolbypassrls, rolsuper INTO r FROM pg_roles WHERE rolname = 'musclex_app';
  IF r.rolbypassrls OR r.rolsuper THEN
    RAISE EXCEPTION 'musclex_app must be NOBYPASSRLS + NOSUPERUSER (got bypassrls=%, super=%)',
      r.rolbypassrls, r.rolsuper;
  END IF;
  RAISE NOTICE 'musclex_app ready: non-bypass, non-super. Cutover = repoint DATABASE_URL + restart.';
END $$;
